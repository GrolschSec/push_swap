...
int	find_best_move_b_to_a(t_stack **a, t_stack **b)
{
	...
	while (tmp)
	{
		if (i > both_up_b_to_a(a, b, tmp->value))
			i = both_up_b_to_a(a, b, tmp->value);
		if (i > both_down_b_to_a(a, b, tmp->value))
			i = both_down_b_to_a(a, b, tmp->value);
		if (i > a_up_b_down_b_to_a(a, b, tmp->value))
			i = a_up_b_down_b_to_a(a, b, tmp->value);
		if (i > a_down_b_up_b_to_a(a, b, tmp->value))
			i = a_down_b_up_b_to_a(a, b, tmp->value);
		tmp = tmp->next;
	}
	return (i);
}
...
int	find_position_a(t_stack **stack, int nb)
{
	...
	if (nb < (*stack)->value && nb > get_last_element(stack)->value)
		...
		i = find_index(stack, find_min(stack));
	else
	{
		cursor_two = (*stack)->next;
		while (cursor_one->value > nb || cursor_two->value < nb)
